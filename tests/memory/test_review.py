...
from test_persistence import RuntimeFixture, _recover
from test_validation import FakeNavigator, _passing_fixture

import bridger.memory.evaluation.review as review_module
from bridger.contracts.memory.core import FindingOrigin, TargetPhase, TargetTaskState
...
def _instructions(fixture: RuntimeFixture) -> ReviewerInstructions:
    return ReviewerInstructions(
        ...
        shared=Path("src/bridger/memory/prompts/reviewer/system.md").read_text(
            encoding="utf-8"
        ),
        target_specific=Path(
            "src/bridger/memory/prompts/reviewer/targets/architecture.md"
