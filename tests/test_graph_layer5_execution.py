...
from graph.enrichment.naming import (
    InvalidCommunityNameBatch,
    validate_community_name_batch,
)
from graph.enrichment.service import _create_community_name_client
...
from models.graph import GraphBuildResult, GraphSnapshotManifest
...
def test_mixed_reuse_generates_only_unmatched_target_and_materializes_new_record(
    ...
    monkeypatch.setattr(
        "graph.enrichment.service._create_community_name_client",
...
def test_layer5_client_disables_internal_retry_budget(
    ...
    monkeypatch.setattr(
        "graph.enrichment.service.create_llm_client_from_profile",
        capture,
    )
...
def _graph_state(
    ...
    *,
    ...
    for target in (
        "graph.enrichment.evidence.load_graph_snapshot_structural_state",
        "graph.enrichment.validation.load_graph_snapshot_structural_state",
        "graph.enrichment.service.load_graph_snapshot_structural_state",
    ):
