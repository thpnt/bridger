...
current candidate-artifact context
unresolved findings
...
| **`WorkerContext`**                                     | Transient contract      | —                                                    | **3 → 4 / 9 → 4**                     | Context compiler                      | Immutable for one compiled cycle              | Exact bounded model-facing view for one worker cycle. Combines instructions, task spec, relevant durable state, completion progress, current artifact context, unresolved findings, selected evidence handles, budgets, and tool definitions. It is **not authoritative state**.       |
