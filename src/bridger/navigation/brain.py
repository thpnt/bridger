"""Bounded retrieval over one published Repository Brain."""
...
class BrainNavigator:
    ...
    def source_identity(self) -> tuple[str, str, str, str | None]:
        """Return the Repository Brain's pinned repository and graph identity."""
        manifest = self._brain.manifest
        return (
            manifest.repository_id,
            manifest.repository_revision,
            manifest.graph_snapshot_id,
            manifest.enrichment_overlay_id,
        )
    ...
    def search_hybrid(
        ...
        *,
        ...
        bm25_candidates = self._bm25_candidates(query)
        dense_candidates = self._dense_candidates(query)
        warnings = (
    ...
    def search_lexical(
        ...
        *,
        ...
        candidates = self._bm25_expression_candidates(expression)
        ranked = self._rank_candidates(
            candidates,
            [],
            hybrid=False,
    ...
    def _bm25_candidates(
        ...
        if expression is None:
            return []
        return self._bm25_expression_candidates(expression)
    ...
    def _dense_candidates(
        ...
        eligible = [
            (float(similarities[index]), self._chunks_by_id[chunk_id])
    ...
    def _rank_candidates(
        ...
        *,
        ...
        candidates.sort(key=lambda item: _candidate_key(item, hybrid=hybrid))
        return candidates
        ...
        *,
        ...
        for document_candidates in by_document.values():
            ...
            for group in groups:
                best = min(
                    group,
                    key=lambda item: _candidate_key(item, hybrid=hybrid),
                )
        ...
        passages.sort(
            key=lambda passage: (
                _candidate_key(passage.best_candidate, hybrid=hybrid),
                passage.start_line,
            )
        )
        return passages
...
def _candidate_key(
    ...
    *,
    ...
    if hybrid:
        relevance: tuple[object, ...] = (-float(candidate.fused_score or 0),)
    else:
        relevance = (
