...
from bridger._vendor.graphify.security import sanitize_label, check_graph_file_size_cap
from bridger._vendor.graphify.build import edge_data, edge_datas
from bridger._vendor.graphify.paths import default_graph_json as _default_graph_json
...
def _build_server(graph_path: str):
    ...
    from bridger._vendor.graphify import paths as _paths
    ...
    def _tool_list_prs(arguments: dict) -> str:
        from bridger._vendor.graphify.prs import fetch_prs, fetch_worktrees, format_prs_text, _detect_default_branch
    ...
    def _tool_get_pr_impact(arguments: dict) -> str:
        from bridger._vendor.graphify.prs import fetch_pr_files, compute_pr_impact, _gh, _parse_ci
    ...
    def _tool_triage_prs(arguments: dict) -> str:
        ...
        from bridger._vendor.graphify.prs import fetch_prs, fetch_worktrees, fetch_pr_files, compute_pr_impact, _STATUS_ORDER, _detect_default_branch
        repo = arguments.get("repo") or None
        base = arguments.get("base") or _detect_default_branch(repo)
        try:
            prs = fetch_prs(repo=repo, base=base)
    ...
    async def read_resource(uri: AnyUrl) -> str:
        ...
        if uri_str == "graphify://surprises":
            try:
                from bridger._vendor.graphify.analyze import surprising_connections
        ...
        if uri_str == "graphify://questions":
            try:
                from bridger._vendor.graphify.analyze import suggest_questions
