...
import sys
import time
from bridger._vendor.graphify.paths import GRAPHIFY_OUT as _GRAPHIFY_OUT
...
def _stale_graph_sources(
    ...
    """
    from bridger._vendor.graphify.paths import nfc
    try:
    ...
    def _provably_excluded(c: Path) -> bool:
        ...
        for s in spellings:
            ...
            if any(s.startswith(pref) for pref in excluded_prefixes):
                return True
        try:
            from bridger._vendor.graphify.detect import _is_sensitive as _det_sensitive
            if _det_sensitive(c):
                return True
        except Exception:
            pass
...
def _prune_graph_json_sources(graph_path: Path, stale_sources: list[str]) -> int:
    """Drop nodes/edges/hyperedges owned by ``stale_sources`` from bridger.graph.json
    ...
    from bridger._vendor.graphify.export import backup_if_protected as _backup
    _backup(graph_path.parent)
    from bridger._vendor.graphify.paths import write_json_atomic
...
def _enforce_graph_size_cap_or_exit(gp: Path) -> None:
    ...
    """
    from bridger._vendor.graphify.security import check_graph_file_size_cap
    try:
...
def _touch_query_stamp(graph_path: "Path") -> None:
    ...
    try:
        from bridger._vendor.graphify.paths import write_text_atomic
...
def _query_stamp_fresh() -> bool:
    ...
    from bridger._vendor.graphify.paths import out_path
    try:
...
def _run_hook_guard(kind: str, strict: bool = False) -> None:
    ...
    """
    from bridger._vendor.graphify.paths import out_path, GRAPHIFY_OUT_NAME
...
def _reenter_main() -> None:
    from bridger._vendor.graphify.__main__ import main
    main()


def dispatch_command(cmd: str) -> None:
    if cmd == "provider":
        from bridger._vendor.graphify.llm import _custom_providers_path, BACKENDS
        ...
        if subcmd == "list":
            ...
            if not base_url or not default_model or not env_key:
                ...
                sys.exit(1)
            from bridger._vendor.graphify.llm import provider_base_url_ok
        ...
        from bridger._vendor.graphify.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from bridger._vendor.graphify.hooks import (
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from bridger._vendor.graphify.serve import _query_graph_text
        from bridger._vendor.graphify.security import sanitize_label
        from networkx.readwrite import json_graph
        from bridger._vendor.graphify import querylog
        ...
        try:
            ...
            try:
                from bridger._vendor.graphify.build import graph_has_legacy_ids as _legacy
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from bridger._vendor.graphify.affected import DEFAULT_AFFECTED_RELATIONS, format_affected, load_graph
        query = sys.argv[2]
        graph_path = _default_graph_path()
        depth = 2
        relations: list[str] = []
        ...
        from bridger._vendor.graphify.affected import load_graph
        from bridger._vendor.graphify.analyze import god_nodes as _god_nodes
        from bridger._vendor.graphify.security import sanitize_label as _sanitize_label
        graph_path = _default_graph_path()
        top_n = 10
        ...
        from bridger._vendor.graphify.ingest import save_query_result as _sqr
        ...
        opts = p.parse_args(sys.argv[2:])
        from bridger._vendor.graphify.reflect import reflect as _reflect, lessons_fresh as _lessons_fresh
        ...
        if len(sys.argv) < 4:
            ...
            sys.exit(1)
        from bridger._vendor.graphify.serve import _pick_scored_endpoint, _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        segments = []
        from bridger._vendor.graphify.build import edge_datas
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from bridger._vendor.graphify.serve import _find_node, find_node_ambiguity
        from networkx.readwrite import json_graph
        ...
        try:
            from bridger._vendor.graphify.reflect import load_learning_overlay as _llo
            from bridger._vendor.graphify.security import sanitize_label as _sl
            _overlay = _llo(gp)
        ...
        from bridger._vendor.graphify.build import edge_data
        ...
        from bridger._vendor.graphify.diagnostics import (
            diagnose_file,
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from bridger._vendor.graphify.ingest import ingest as _ingest
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from bridger._vendor.graphify.watch import watch as _watch
        ...
        from bridger._vendor.graphify.build import build_from_json
        from bridger._vendor.graphify.cluster import cluster, score_all, remap_communities_to_previous
        from bridger._vendor.graphify.analyze import (
            god_nodes,
            surprising_connections,
            suggest_questions,
        )
        from bridger._vendor.graphify.report import generate
        from bridger._vendor.graphify.export import to_json, to_html
        ...
        from bridger._vendor.graphify.security import check_graph_file_size_cap as _check_cap
        _over_cap = False
        try:
        ...
        if labels_path.exists() and not force_relabel:
            ...
            from bridger._vendor.graphify.cluster import community_member_sigs, label_communities_by_hub
            ...
            from bridger._vendor.graphify.cluster import label_communities_by_hub
            from bridger._vendor.graphify.llm import generate_community_labels
        ...
        from bridger._vendor.graphify.export import _git_head as _gh
        _commit = _gh()
        from bridger._vendor.graphify.report import load_learning_for_report as _llfr
        ...
        if not placeholder_only:
            from bridger._vendor.graphify.paths import write_json_atomic as _wja
            ...
            # (see reuse above).
            from bridger._vendor.graphify.cluster import community_member_sigs as _cms
        ...
        if not watch_path.exists():
            print(f"error: path not found: {watch_path}", file=sys.stderr)
            sys.exit(1)
        from bridger._vendor.graphify.watch import _rebuild_code
        ...
        if len(sys.argv) < 3:
            ...
            sys.exit(1)
        from bridger._vendor.graphify.watch import check_update
        ...
        from bridger._vendor.graphify.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        ...
        def _load_graph(p: str):
            ...
            from bridger._vendor.graphify.paths import load_node_link_graph as _lnlg
        ...
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from bridger._vendor.graphify.build import prefix_graph_for_global as _prefix, distinct_repo_tags as _repo_tags
        graphs = []
        ...
        if subcmd == "callflow-html":
            from bridger._vendor.graphify.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from bridger._vendor.graphify.build import build_from_json as _bfj
        from bridger._vendor.graphify.security import check_graph_file_size_cap as _check_cap
        ...
        if subcmd == "html":
            from bridger._vendor.graphify.export import to_html as _to_html
            if no_viz:
            ...
            from bridger._vendor.graphify.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            from bridger._vendor.graphify.wiki import to_wiki as _to_wiki
            from bridger._vendor.graphify.analyze import god_nodes as _god_nodes
            if not communities:
                print(
            ...
            if push_uri:
                ...
                from bridger._vendor.graphify.export import to_cypher as _to_cypher
            ...
            if push_uri:
                from bridger._vendor.graphify.export import push_to_falkordb as _push
                result = _push(G, uri=push_uri, user=push_user,
                               password=push_password, communities=communities)
        ...
        from bridger._vendor.graphify.benchmark import run_benchmark, print_benchmark
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from bridger._vendor.graphify.global_graph import (
        ...
        from bridger._vendor.graphify.watch import (
        ...
        from bridger._vendor.graphify.detect import (
            detect as _detect,
        ...
        from bridger._vendor.graphify.llm import (
        ...
        if needs_llm:
            if backend is None:
                ...
                sys.exit(1)
            if backend == "ollama":
                from bridger._vendor.graphify.llm import _validate_ollama_base_url
        ...
        if code_files:
            from bridger._vendor.graphify.extract import extract as _ast_extract
        ...
        from bridger._vendor.graphify.cache import (
        ...
        from bridger._vendor.graphify.llm import _extraction_system as _sem_prompt_for
        ...
        try:
            from bridger._vendor.graphify.cache import file_hash as _file_hash
        ...
        if cli_postgres_dsn is not None:
            from bridger._vendor.graphify.pg_introspect import introspect_postgres
        ...
        if cli_cargo:
            from bridger._vendor.graphify.cargo_introspect import introspect_cargo
        ...
        if no_cluster:
            ...
            from bridger._vendor.graphify.build import dedupe_edges as _dedupe_edges, dedupe_nodes as _dedupe_nodes
            from bridger._vendor.graphify.export import (
            ...
            if incremental_mode:
                ...
                from bridger._vendor.graphify.build import merge_raw_extraction as _merge_raw_extraction
            ...
            from bridger._vendor.graphify.build import disambiguate_file_labels_in_nodes as _disamb_labels
            ...
            if _extraction_incomplete and not cli_allow_partial:
                from bridger._vendor.graphify.export import MALFORMED_GRAPH as _MALFORMED_GRAPH
            ...
            from bridger._vendor.graphify.paths import write_json_atomic as _write_json_atomic
            ...
            try:
                ...
                print(f"[graphify extract] warning: could not write manifest: {exc}", file=sys.stderr)
            if global_merge:
                from bridger._vendor.graphify.global_graph import global_add as _global_add
        ...
        from bridger._vendor.graphify.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from bridger._vendor.graphify.cluster import cluster as _cluster, score_all as _score_all
        from bridger._vendor.graphify.export import to_json as _to_json
        from bridger._vendor.graphify.analyze import god_nodes as _god_nodes, surprising_connections as _surprising
        ...
        from bridger._vendor.graphify.cache import check_semantic_cache
        if len(sys.argv) < 3:
        ...
        from bridger._vendor.graphify.semantic_cleanup import load_validated_semantic_fragment
